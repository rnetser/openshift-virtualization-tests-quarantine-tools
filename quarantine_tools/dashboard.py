...
Part of the quarantine_tools package -- a standalone tool for scanning
...
class TestInfo(NamedTuple):
    ...
    Attributes:
        ...
        quarantine_age_days: Days since the xfail QUARANTINED marker was added to this file.
        failure_rate: Failure rate from ReportPortal (0.0 to 1.0). Defaults to 0.0 when unavailable.
...
class DashboardStats(NamedTuple):
    ...
    Attributes:
        ...
        category_breakdown: Dict mapping category name to counts and metrics
            ({"total": N, "active": N, "quarantined": N, "avg_quarantine_age_days": F}).
        quarantined_list: List of TestInfo for all quarantined tests.
        flaky_candidates: List of TestInfo for tests with high failure rate that are NOT quarantined.
    ...
    flaky_candidates: list[TestInfo] = []
...
class TestScanner:
    ...
    def _scan_file(self, file_path: Path) -> list[TestInfo]:
        ...
        # Calculate quarantine age for the file (shared across all quarantined tests in same file)
        # Only compute if the file contains QUARANTINED markers to avoid unnecessary git commands
        has_quarantine_marker = "QUARANTINED" in content
        quarantine_age = self._get_quarantine_age_days(file_path=file_path) if has_quarantine_marker else 0
        ...
        for node in walk(tree):
            if isinstance(node, FunctionDef) and node.name.startswith("test_"):
                ...
                test_info = TestInfo(
                    ...
                    jira_ticket=jira,
                    quarantine_age_days=quarantine_age if is_quarantined else 0,
                )
                tests.append(test_info)

        return tests

    def _get_quarantine_age_days(self, file_path: Path) -> int:
        """Calculate the number of days since the QUARANTINED marker was added to a file.

        Uses git log with the -S flag to find the commit that first introduced the
        QUARANTINED string in the given file.

        Args:
            file_path: Path to the test file containing the QUARANTINED marker.

        Returns:
            Number of days since the QUARANTINED marker was added. Returns 0 if git
            is unavailable, the file is not tracked, or the date cannot be determined.
        ...
        try:
            commit_date = datetime.fromisoformat(date_string)
            age_delta = datetime.now(tz=UTC) - commit_date.astimezone(tz=UTC)
            return max(0, age_delta.days)
        except ValueError:
            LOGGER.warning("Invalid date format from git log for %s: %s", file_path, date_string)
            return 0
    ...
    def _calculate_stats(self, all_tests: list[TestInfo], flaky_threshold: float = 0.0) -> DashboardStats:
        """Calculate aggregated statistics from list of tests.

        Groups tests by category and counts active vs quarantined tests.
        Calculates average quarantine age per category and identifies flaky candidates.
        ...
        Returns:
            DashboardStats with totals, breakdowns, quarantined list, and flaky candidates.
        ...
        # Category breakdown with quarantine age tracking
        category_breakdown: dict[str, dict[str, int | float]] = defaultdict(
            lambda: {"total": 0, "active": 0, "quarantined": 0, "avg_quarantine_age_days": 0.0}
        )
        category_age_totals: dict[str, list[int]] = defaultdict(list)

        for test in all_tests:
            category_breakdown[test.category]["total"] += 1
            if test.is_quarantined:
                category_breakdown[test.category]["quarantined"] += 1
                category_age_totals[test.category].append(test.quarantine_age_days)
            else:
                category_breakdown[test.category]["active"] += 1

        # Calculate average quarantine age per category
        for category, ages in category_age_totals.items():
            if ages:
                category_breakdown[category]["avg_quarantine_age_days"] = round(sum(ages) / len(ages), 1)

        # Identify flaky candidates: non-quarantined tests with failure rate above threshold
        flaky_candidates: list[TestInfo] = []
        if flaky_threshold > 0.0:
            flaky_candidates = [
                test for test in all_tests
                if not test.is_quarantined and test.failure_rate >= flaky_threshold
            ]
            flaky_candidates = sorted(flaky_candidates, key=lambda test: test.failure_rate, reverse=True)
...
class DashboardGenerator:
    ...
    def __init__(
        ...
        Args:
            ...
            with_reportportal: Whether ReportPortal data is available for flaky test detection.
        ...
        self.with_reportportal = with_reportportal
    ...
    def generate(self) -> str:
        ...
        detailed quarantined tests section, and optionally a flaky tests tab.
...
{self._generate_quarantined_html()}
        ...
        # Add flaky tests section when ReportPortal data is available
        if self.with_reportportal:
            parts.append(self._generate_flaky_tests_section())

        parts.append(self._render_footer(timestamp=timestamp))
    ...
    def _render_css(self) -> str:
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .age-badge.age-green { background: #dcfce7; color: #166534; }
        .age-badge.age-yellow { background: #fef9c3; color: #854d0e; }
        .age-badge.age-red { background: #fee2e2; color: #991b1b; }
        .flaky-item {
            padding: 0.75rem 1rem;
        ...
        .flaky-item code { background: #fef08a; padding: 0.125rem 0.375rem; border-radius: 4px; font-size: 0.875rem; }
        .flaky-item .meta { color: var(--gray); font-size: 0.875rem; margin-top: 0.25rem; }
        .failure-rate {
            display: inline-block;
            padding: 0.125rem 0.5rem;
            border-radius: 12px;
        ...
        for team in sorted_teams:
            ...
            for rvs in repo_version_pairs:
                ...
                if category_data is None:
                    ...
                    quarantined = category_data.get("quarantined", 0)
                    avg_age = category_data.get("avg_quarantine_age_days", 0.0)
                    ...
                    avg_age_title = f" (avg {avg_age:.0f}d)" if avg_age else ""
                    tooltip = f' title="Avg quarantine age: {avg_age:.0f} days"' if avg_age else ""
                    cells.append(
                        f'<td><span class="{cell_class}"{tooltip}>{quarantined}{avg_age_title}</span></td>'
                    )
...
{rows_html}
    ...
    @staticmethod
    def _get_age_badge_html(age_days: int) -> str:
        """Generate an HTML badge for quarantine age with color coding.

        Args:
            age_days: Number of days the test has been quarantined.

        Returns:
            HTML string for the color-coded age badge.

        """
        if age_days < 14:
            age_class = "age-green"
        elif age_days <= 30:
            age_class = "age-yellow"
        else:
            age_class = "age-red"
        return f'<span class="age-badge {age_class}">{age_days}d</span>'
    ...
    def _generate_quarantined_html(self) -> str:
        ...
        HTML blocks for each test with Jira links, file locations, and quarantine age.
        ...
        for category in sorted(by_category.keys()):
            ...
            avg_age = self.stats.category_breakdown.get(category, {}).get("avg_quarantine_age_days", 0.0)
            avg_age_display = f" | Avg age: {avg_age:.0f}d" if avg_age else ""
            lines.append(
                f'            <div class="team-header">{category_display}'
                f" ({test_count} test{plural}{avg_age_display})</div>",
            )
            ...
            for test in sorted(tests, key=lambda test_item: test_item.name):
                ...
                age_html = f" {self._get_age_badge_html(age_days=test.quarantine_age_days)}"

                lines.append(f"""            <div class="test-item">
                <code>{escaped_name}</code>{jira_html}{age_html}
...
"""
    ...
    def _generate_flaky_tests_section(self) -> str:
        """Generate HTML section for flaky test candidates.

        Shows tests that have a high failure rate from ReportPortal but are not yet quarantined.
        This section is only populated when --with-reportportal flag is used and ReportPortal
        data is available.

        Returns:
            HTML string containing the flaky tests section, or an informational note
            if no flaky candidates are found.

        """
        # Collect all flaky candidates across all repos and versions
        all_flaky: list[tuple[str, str, TestInfo]] = []  # (repo, branch, test)

        if self.repo_stats:
            for repo, version_stats_list in self.repo_stats.items():
                short_repo = repo.rsplit("/", maxsplit=1)[-1]
                for version_stat in version_stats_list:
                    for test in version_stat.stats.flaky_candidates:
                        all_flaky.append((short_repo, version_stat.branch, test))
        elif self.stats.flaky_candidates:
            for test in self.stats.flaky_candidates:
                all_flaky.append(("", self.branch, test))

        section_parts = ['        <div class="section">', "            <h2>Flaky Test Candidates</h2>"]

        if not all_flaky:
            section_parts.append(
                '            <div class="note">No flaky test candidates detected. '
                "Tests are considered flaky when their ReportPortal failure rate exceeds the configured threshold "
                "and they are not yet quarantined.</div>"
            )
        else:
            flaky_count = len(all_flaky)
            section_parts.append(
                f"            <p>Found <strong>{flaky_count}</strong> test(s) with high failure rates "
                "that are not yet quarantined:</p>"
            )

            by_category: dict[str, list[tuple[str, str, TestInfo]]] = defaultdict(list)
            for repo, branch, test in all_flaky:
                by_category[test.category].append((repo, branch, test))
            ...
            for category in sorted(by_category.keys()):
                ...
                section_parts.append(
                    f'            <div class="team-header">{category_display}'
                    f" ({test_count} test{plural})</div>",
                )

                for repo, branch, test in sorted(category_tests, key=lambda item: -item[2].failure_rate):
                    rel_path = html_escape(s=self._get_display_path(file_path=test.file_path))
                    escaped_name = html_escape(s=test.name)
                    failure_pct = ceil(test.failure_rate * 100)
                    rate_html = f'<span class="failure-rate">{failure_pct}% failures</span>'
                    location_parts = [f"File: {rel_path}:{test.line_number}"]
                    if repo:
                        location_parts.append(f"Repo: {repo}")
                    location_parts.append(f"Branch: {branch}")
                    location_meta = " | ".join(location_parts)

                    section_parts.append(f"""            <div class="flaky-item">
                <code>{escaped_name}</code> {rate_html}
                <div class="meta">{location_meta}</div>
            </div>""")
...
def generate_json_output(repo_stats: dict[str, list[VersionStats]]) -> str:
    ...
    for repo, version_stats_list in repo_stats.items():
        ...
        for version_stat in version_stats_list:
            ...
            teams_data: dict[str, dict[str, int | float]] = {}
            for team, counts in stats.category_breakdown.items():
                teams_data[team.replace("_", " ").title()] = {
                    "total": counts["total"],
                    "active": counts["active"],
                    "quarantined": counts["quarantined"],
                    "avg_quarantine_age_days": counts.get("avg_quarantine_age_days", 0.0),
                }
            ...
            for test in stats.quarantined_list:
                quarantined_tests.append({
                    ...
                    "quarantine_age_days": test.quarantine_age_days,
                    "failure_rate": test.failure_rate,
                })

            # Build flaky candidates list
            flaky_tests: list[dict] = []
            for test in stats.flaky_candidates:
                flaky_tests.append({
                    "name": test.name,
                    "file": get_display_path(file_path=test.file_path),
                    "line": test.line_number,
...
Examples:
    ...
    quarantine-dashboard --workdir /path/to/clones --output-dir /path/to/output
        """,
    )
    parser.add_argument(
        "--keep-clones",
